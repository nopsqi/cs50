...
def index():
    ...
    return render_template("index.html", cash=cash, datas=datas, total=sum(data["total"] for data in datas) + cash, usd=usd)
...
def history():
    ...
    datas = db.execute("SELECT h.transacted, h.price, h.shares, s.symbol FROM histories h JOIN symbols s ON s.id = h.symbol_id WHERE user_id = ?;", session["user_id"])
...
def sell():
    ...
    rows = db.execute("SELECT p.shares, s.id, s.symbol, s.name FROM portofolios p JOIN symbols s ON s.id = p.symbol_id WHERE p.user_id = ? AND p.shares > 0 GROUP BY p.symbol_id;", session["user_id"])
    if request.method == "POST":
        if not request.form.get("symbol") and not request.form.get("shares"):
            return apology("all field empty.")
        if not request.form.get("symbol"):
